# -*- coding: utf-8 -*-
"""
Created on Fri Feb 20 16:34:47 2026
...
class CRUDWindow(tk.Toplevel):
    ...
    def add(self):
         ...
         for col in self.columns[1:]:
             val=simpledialog.askstring("Input",f"{col}")
             if val is None:
                 return
             if self.table=="students" and col=="password":
                 val=hash_pwd(val)
             if col in ["credit_unit","max_score","score"]:
                 try: val=float(val)
    ...
    def edit(self):
        ...
        for i,col in enumerate(self.columns[1:],start=1):
            val=simpledialog.askstring("Edit",f"{col}",initialvalue=str(vals[i]))
            ...
            new_vals.append(val)
        set_clause=",".join([f"{c}=?" for c in self.columns[1:]])
        self.db.cursor.execute(f"UPDATE {self.table} SET {set_clause} WHERE id=?",(*new_vals,vals[0]))
        self.db.conn.commit()
        self.refresh()
    def delete(self):
...
class AppController(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("ACADEMIC PERFORMANCE MANAGEMENT SYSTEM (ANGELNAITBJ®)")
